...
##print 's/w: %f' %(s/w)
##print float(l/w)
arr = np.full([480/100, 640/100], 100, np.uint8)

for i_g in range(0, 640, 100):
    a_g=i_g+w
    g1+=1
    
    for j_g in range(0, 480 , 100):
        b_g+=w
        g2+=1
        for nx in range(3,i):
            ...
            if(vx[nx]>i_g and vx[nx]<i_g+w and vy[nx]>j_g and vy[nx]<j_g+h):
                arr[j_g/w,i_g/w] = 255
