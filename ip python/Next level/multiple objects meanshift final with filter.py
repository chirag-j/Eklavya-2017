...
thresh1 = np.zeros([obj_lim, a, b], np.uint8)
thresh = np.zeros([obj_lim, a, b, 3], np.uint8)
...
while(1):
    ...
    for n in range(0,i):
        ...
        #ret,thresh1[n] = cv2.threshold(dst[n],50,255,0)
        #thresh[n] = cv2.merge((thresh1[n],thresh1[n],thresh1[n]))
        #
        #cv2.imshow('dst1', thresh[1])
        #cv2.imshow('dst0', thresh[0])
        #cv2.imshow('dst1', dst[1])
...
##        img[n] = cv2.rectangle(img[n-1], (x[n],y[n]), (x[n]+w[n],y[n]+h[n]), 255,2)
        cv2.rectangle(frame, (x[n],y[n]), (x[n]+w[n],y[n]+h[n]), 255,2)
