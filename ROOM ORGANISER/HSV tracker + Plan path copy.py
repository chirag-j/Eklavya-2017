...
while True:
    ...
    if k == 27:
        ...
        if perm_obs_pos == True:
            del px1[ipop-1]
            del px2[ipop-1]
            del py1[ipop-1]
            del py2[ipop-1]
            ipop-=1
            print "Obstacle Id no : ", ipop, "removed"
        else:
            del ux[ir-1]
            del uy[ir-1]
            del vx[ir-1]
...
for xg in range(0, 640, wb):
    ...
    for yg in range(0, 480, wb):
        ...
        bg+=wb
map_array[int(cy[ir-1]/wb), int(cx[ir-1]/wb)] = 0
print map_array
if (float(640/wb) - int(640/wb)) > 0.5 and (float(480/wb) - int(480/wb)) > 0.5:
    pass
elif (float(640/wb) - int(640/wb)) < 0.5 and (float(480/wb) - int(480/wb)) > 0.5:
    map_array1 = map_array[0:ary, 0:arx-1]
elif (float(640/wb) - int(640/wb)) > 0.5 and (float(480/wb) - int(480/wb)) < 0.5:
    map_array1 = map_array[0:ary-1, 0:arx]
else:
    print (float(640/wb) - int(640/wb)), (float(480/wb) - int(480/wb))
    map_array1 = map_array[0:ary-1, 0:arx-1]
print arx, ary
print map_array1
fill(map_array1)
print map_array1

ptp = plan_path(map_array1, cy[0]/wb, cx[0]/wb)
print ptp
ptps = []
for d in range(0, len(ptp)):
    ptps.append(((ptp[d,0]*wb)+ wb/2 , (ptp[d,1]*wb)+ wb/2))
ptps = np.int0(ptps)
cv2.polylines(mask_map, [ptps], False, 255, 2)
cv2.imshow('mask_map', mask_map)
clamp_close = False
for xc, yc in ptps:
    while cx[0]<xc-(wb/2) or cx[0]>xc+(wb/2) or cy[0]<yc-(wb/2) or cy[0]>yc+(wb/2):
        frame = cap.read()
        frame_h = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        for ls in range(0,ir):
            mask[ls] = cv2.inRange(frame_h, lower_range[ls], upper_range[ls])
